...
def play_game(sum_total, game_move, first_move, name_1, name_2):
    while sum_total > 1:
        while first_move == name_1:
            ...
            n = random.randint(1, game_move)
            print(f'{name_1} забирает {n} конфет(-у/-ы).')
            sum_total -= int(n)
            if sum_total == 1:
                return print(Fore.YELLOW + Back.GREEN +
                             f'{name_1} выиграл! Его противнику {name_2} осталась 1 конфета. Поздравляем, {name_1}')
            print(
                f'После хода {name_1}а осталось {sum_total} конфет(-а)')
            first_move = name_2
